...
const audioDir = "audios-examen/";

const container = document.getElementById("photo-question-container");
...
// === DÉMARRAGE ===
fetch("examen-photographies.json?v=" + Date.now())
  .then(res => res.json())
  .then(data => {
    // Mélanger et sélectionner toutes les questions de photographies
...
function loadQuestion() {
  ...
  if (q.image) {
    ...
    img.src = q.image;
    img.alt = `Photo de la question ${questionIndex + 1}`;
...
function showFinalScore() {
  ...
  .then(data => {
    if (data.success) {
      console.log("Score de photographies sauvegardé ! Score total:", data.score_total);
    }
  })
